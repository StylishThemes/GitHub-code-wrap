...
* Try to limit the style size:
  * Don't add any image URI's to the css; instead add the image into the `/images` directory; then point to using the following url: `http://stylishthemes.github.io/GitHub-code-wrap/images/`{my-image.png}.
...
* [Download](https://github.com/StylishThemes/GitHub-code-wrap/archive/master.zip), [fork](https://github.com/StylishThemes/GitHub-code-wrap/fork) or clone this repository.
* Use [node.js](http://nodejs.org/) to run `npm install`.
* Make any changes to the `github-code-wrap.user.css` file and save.

### Build & test

* Create & change into a new branch of your local GitHub-code-wrap repository.
...
* Now you can add and commit the changes of the `github-code-wrap.user.css` file to your fork's branch.
